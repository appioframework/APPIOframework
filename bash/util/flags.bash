...
function should_run_system_test() {
    if $( are_empty_args ${@} );
    then
        exit 0
    fi
    ...
    for ARG in ${@}
    do
        if [ "${ARG}" == "${FLAG_SYSTEM_TEST}" ] || [ "${ARG}" == "${FLAG_FULL_PIPELINE}" ];
        then
            exit 0
        fi
    done
...
function print_condition_for_system_test() {
    echo "${FLAG_SYSTEM_TEST} or ${FLAG_FULL_PIPELINE}"
}
