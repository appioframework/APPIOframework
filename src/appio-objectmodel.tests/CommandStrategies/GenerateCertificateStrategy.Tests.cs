...
namespace Appio.ObjectModel.Tests.CommandStrategies
{
    public class GenerateCertificateStrategyTests
    {
        ...
        private const string AppioprojFile = "appioproj-file";
        ...
        public void Cleanup()
        {
            AppioLogger.RemoveListener(_listenerMock.Object);
        }
        ...
        public void GenerateCorrectCertificateOnValidInput(string[] inputParams, string expectedAppName, uint expectedKeySize, uint expectedDays, string expectedOrg)
        {
            ...
            var stream = MockFileRead(AppioprojFile, SampleClientAppProject);
        ...
        public void GenerateCorrectCertificateOnValidInputWithClientServerApp(string[] inputParams, string expectedAppName, uint expectedKeySize, uint expectedDays, string expectedOrg)
        {
            // arrange
            MockPathCombine(expectedAppName);
            var stream = MockFileRead(AppioprojFile, SampleClientServerAppProject);
        ...
        public void FailWhenProjectNotFound()
        {
            ...
            _fileSystemMock.Setup(fs => fs.ReadFile(AppioprojFile)).Throws<Exception>();
        ...
        private void MockPathCombine(string appName)
        {
            ...
            _fileSystemMock.Setup(fs => fs.CombinePaths(appName, appName + Constants.FileExtension.Appioproject)).Returns(AppioprojFile);
        }
    }
}